...
%
function [precision, fps] = run_tracker1(video, show_visualization, show_plots)
dbstop if error;
%path to the videos (you'll be able to choose one with the GUI).
base_path   = 'E:\lu Xiankai\vot100\'; %
addpath(genpath('E:\lu Xiankai\toolbox-master\'));
close all;
addpath('utility');
addpath('model');
result_path = 'results100\';
...
config.motion_thresh= 0.1; %singer2 0.1807, girl2 0.1886 ,0.189 for all
config.appearance_thresh=0.3; %0.38
...
switch video
    ...
    case 'all',
        ...
        if nonzeros(strcmp(videos,'Jogging'))
                videos(strcmpi('Jogging', videos)) = [];
                videos(end+1:end+2) = {'Jogging.1', 'Jogging.2'};
        end
        if nonzeros(strcmp(videos,'Skating2'))
...
%             cell_size, show_visualization);
        [positions, time,rect_position] = tracker_CF_RP_train_new1(video_path, img_files, pos, target_sz, ...
            padding, lambda, output_sigma_factor, interp_factor, ...
        ...
        results.len = size(rect_position,1);
        results.fps = fps; 
...
%             mkdir(result_path);
%         end
        video(1) = lower(video(1));
        save([result_path  video '_' 'CFRP12' '.mat'],'results');
       % save([result_path  video '_' 'CN_rp_prec' '.mat'],'precision');
        if nargout > 0,
