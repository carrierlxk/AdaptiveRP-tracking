function [positions, time,rect_position] = tracker_CF_RP_train_new1(video_path, img_files, pos, target_sz, ...
...
for frame = 1:numel(img_files),
    ...
    if frame > 1
        ...
        feat1 = extractFeature1(im, pos, app_sz,cos_window1, config);
        zf = fft2(feat1);
        kzf=gaussian_correlation(zf, app_xf, config.kernel_sigma);
        app_alpha = real(ifft2(app_alphaf));
        response = app_alpha.*ifft2(kzf);
        max_response = sum(response(:));
        % motion response
        patch = get_subwindow(im, pos, current_window_sz);
        patch = imresize(patch,config.window_sz,'bilinear');
% Extracting hierarchical convolutional features
        feat2  = my_get_features(patch, config,cos_window);
...
%         max_response
        if  max_response< config.motion_thresh,
               [patch1,~] = my_get_subwindow((im),pos,window_sz); 
            ...
            if (nonzeros(ind))
              ...
              %patch2 = patch1(bb(2):1:bb(2)+bb(4),bb(1):1:(bb(1)+bb(3)),:);                
                    ...
                    feat1 =  my_get_features(patch2,config,cos_window);    
                ...
                if motion_response<max(responind(:))                
                   ...
                   bb = bbs(ind(indx),:);      
                    if  bb(4)*bb(3)< 1.2*(prod(current_target_sz)) &&bb(4)*bb(3)>0.7*(prod(current_target_sz))
                      ...
                      pos = pos+floor( 0.3*(pos3-pos));                     
                      current_target_sz = current_target_sz+(0.5*([bb(4) bb(3)]- current_target_sz));
                    end
                end
           end
          end   
    ...
    sxf = fft2(my_get_features(patch, config,cos_window)); %feat  = my_get_features(patch, config,cos_window);
...
function feat  = extractFeature1(im, pos, window_sz, cos_window, config)

% Get the search window from previous detection
patch = get_subwindow(im, pos, window_sz);
% Extracting hierarchical convolutional features
feat  = my_get_features(patch, config,cos_window);
